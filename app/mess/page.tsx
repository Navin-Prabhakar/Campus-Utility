...
const MESS_CONFIG = [
  ...
  { id: "mess2", name: "Aryabhatta (5)", gid: "1633328713", contact: "916200202614" }, 
...
export default function MessPage() {
    ...
    Papa.parse(GOOGLE_SHEET_CSV_URL, {
      ...
      complete: (results) => {
        ...
        for (let i = 3; i < rows.length; i++) {
          ...
          if (!dayName || dayName.toUpperCase() === "EVERYDAY") continue;
  ...
  // Helper utility function to convert real local binary file nodes into asynchronous Base64 strings
...
const handleLogDataToSheets = async (e: React.FormEvent) => {
    ...
    try {
      const base64ImagesArray = await Promise.all(selectedImages.map(file => convertFileToBase64(file)));

      // 🟢 Force a raw POST request with cross-origin safety configurations
      await fetch(COMPLAINT_SCRIPT_URL, {
        method: "POST",
        mode: "no-cors", 
        body: JSON.stringify({
          messName: selectedMess.name, // Must match your spreadsheet tab name exactly (case-sensitive)
      ...
      alert("Network transmission failure. Check deployment URL link.");
    }
  ...
  return (
    <main className="h-screen max-h-screen w-full bg-zinc-100 font-sans text-zinc-600 antialiased flex flex-col items-center justify-between relative overflow-hidden">
      
      <div className="w-full flex flex-col items-center">
        <Header messActionSlot={HeaderDropdownAction} />

        <main className="flex flex-col items-center justify-start py-4 w-[92%] max-w-[350px]">
          ...
          {!selectedMess ? (
            ...
            <div className="w-full space-y-2.5">
              <div className="w-full rounded-xl border border-zinc-300 bg-white p-2 shadow-xs">
                <div className="mb-2 flex items-center justify-between border-b border-zinc-500 pb-1 px-1">
                  <div className="flex items-center gap-1">
                    <span className="h-1 w-1 rounded-full bg-emerald-500 animate-pulse" />
                    <h2 className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Today's Layout</h2>
                  </div>
                    ...
                    <div className="flex flex-col gap-1.5 p-0.5">
                      <div className="flex items-center justify-between rounded-md border border-zinc-200 bg-zinc-200/30 px-2.5 py-1.5">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-extrabold text-blue-600 uppercase tracking-wider">Breakfast</span>
                          <span className="text-[11px] font-bold text-zinc-800 mt-0.5 leading-snug">{todaysMenu.Breakfast || "—"}</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between rounded-md border border-zinc-200 bg-zinc-200/30 px-2.5 py-1.5">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-extrabold text-blue-600 uppercase tracking-wider">Lunch</span>
                          <span className="text-[11px] font-bold text-zinc-800 mt-0.5 leading-snug">{todaysMenu.Lunch || "—"}</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between rounded-md border border-zinc-200 bg-zinc-200/30 px-2.5 py-1.5">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-extrabold text-blue-600 uppercase tracking-wider">Snacks</span>
                          <span className="text-[11px] font-bold text-zinc-800 mt-0.5 leading-snug">{todaysMenu.Snacks || "—"}</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between rounded-md border border-zinc-200 bg-zinc-200/30 px-2.5 py-1.5">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-extrabold text-blue-600 uppercase tracking-wider">Dinner</span>
                ...
                <div className="mt-2 px-0.5">
                  <a 
                    ...
                    className="flex w-full items-center justify-center rounded-md bg-zinc-800 py-1.5 text-[11px] font-bold text-white transition-colors hover:bg-zinc-900"
                  >
                    View Full Schedule Spreadsheet
                  </a>
                </div>
      ...
      {selectedMess && (
        <button
          ...
          className="fixed bottom-20 right-4 h-12 w-12 rounded-full bg-linear-to-br from-rose-500 to-red-600 text-white font-bold text-lg flex items-center justify-center shadow-[0_4px_10px_rgba(225,29,72,0.4),inset_0_2px_3px_rgba(255,255,255,0.3)] border border-rose-600 z-50 cursor-pointer transition transform active:scale-90"
        >
          🚨
        </button>
      )}
